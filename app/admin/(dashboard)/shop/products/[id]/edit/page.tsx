...
export default function EditProductPage() {
  ...
  const [images, setImages] = useState<(string | { file: File; preview: string })[]>([]);
  ...
  const [colors, setColors] = useState({ 
    ...
    items: [] as { 
      name: string; 
      hex: string; 
      shortImage?: string | { file: File; preview: string }; 
      longImage?: string | { file: File; preview: string } 
    }[] 
  });
  const [sizes, setSizes] = useState({ 
    enabled: false, 
    sizeGuide: null as string | { file: File; preview: string } | null, 
    items: [
      { name: "S", active: false },
      { name: "M", active: true },
      { name: "L", active: true },
      ...
      if (productId) {
        ...
        if (product) {
          reset({
            ...
            detailMaterial: product.detail_material || "",
            shippingInfo: product.shipping_info || "",
            stock: product.stock?.toString() || "0",
            isPreOrder: product.is_preorder || false,
            preOrderType: product.preorder_data?.type || "days",
            preOrderValue: product.preorder_data?.value || ""
          });
          
          setImages(product.images || (product.image ? [product.image] : []));
          setSelectedCategories(product.categories || []);
          
          if (product.variants?.colors) setColors(product.variants.colors);
          if (product.variants?.sizes) setSizes(product.variants.sizes);
          if (product.variants?.sleeves) setSleeves(product.variants.sleeves);
          
          if (product.variants?.matrix) {
            // Check if existing matrix is old format (just numbers) or new format (objects)
            const oldMatrix = product.variants.matrix;
            const newMatrix: Record<string, { stock: number, price: string }> = {};
            
            Object.keys(oldMatrix).forEach(key => {
              const val = oldMatrix[key];
              if (typeof val === 'number') {
                newMatrix[key] = { stock: val, price: "0" };
              } else {
                newMatrix[key] = { 
                    ...val, 
                    price: val.price ? new Intl.NumberFormat("id-ID").format(val.price) : "0" 
                };
              }
            });
  ...
  const uploadFile = async (item: string | { file: File; preview: string }) => {
  ...
  const onSubmit = async (values: ProductFormValues) => {
    ...
    try {
      ...
      let sizeGuideUrl = "";
      if (sizes.sizeGuide) {
        sizeGuideUrl = await uploadFile(sizes.sizeGuide);
      }
      ...
      const updatedColorItems = await Promise.all(
        colors.items.map(async (item) => {
          let shortImageUrl = "";
          let longImageUrl = "";
          if (item.shortImage) shortImageUrl = await uploadFile(item.shortImage);
          if (item.longImage) longImageUrl = await uploadFile(item.longImage);
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        ...
        <span className="font-bold uppercase text-[10px] tracking-[0.3em]">Synthesizing Unit ID #{productId}...</span>
    </div>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto pb-32">
      <div className="flex items-center gap-6 mb-12">
        ...
        <div>
          <Text as="h1" className="text-6xl font-hand m-0 tracking-normal">Mod Unit Record</Text>
          <Text className="text-[10px] uppercase font-bold tracking-widest opacity-40">System Identity: {productId}</Text>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-12">
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                  {images.map((img, idx) => (
                    <div key={idx} className="aspect-square bg-white border-2 border-black relative group overflow-hidden">
                        <img src={typeof img === 'string' ? img : img.preview} alt="Product" className="w-full h-full object-cover" />
                        <button type="button" onClick={() => removeImage(idx)} className="absolute inset-0 bg-red-600/90 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity font-black uppercase text-[9px]">TERMINATE</button>
                    </div>
                  ))}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                  {categoriesList.map((cat) => (
                      <button key={cat.id} type="button" onClick={() => toggleCategory(cat.name)} className={cn("px-3 py-2 text-[9px] font-black border-2 border-black uppercase transition-all", selectedCategories.includes(cat.name) ? "bg-black text-white" : "bg-white text-black hover:bg-zinc-100")}>{cat.name}</button>
                  ))}
              </div>
            </div>
          </div>
        ...
        <div className={sectionClass}>
          ...
          <div className="space-y-6">
            <div>
              ...
              <input {...register("name")} className={cn(retroInputClass, errors.name && "border-red-600")} placeholder="UNIT DESIGNATION" />
                                    ...
                                    {sizes.sizeGuide ? (
                                        <>
                                            <div className="w-12 h-12 bg-zinc-200 border-2 border-black overflow-hidden shrink-0">
                                                <img src={typeof sizes.sizeGuide === 'string' ? sizes.sizeGuide : sizes.sizeGuide.preview} className="w-full h-full object-cover" />
                                            </div>
                            ...
                            <div key={idx} className="border-2 border-black p-4 bg-white relative">
                                ...
                                <div className="flex flex-col gap-4">
                                    <div className="flex gap-2">
                                        ...
                                        <input type="text" placeholder="COLOR IDENTITY" value={c.name} onChange={(e) => setColors(p => ({...p, items: p.items.map((item, i) => i === idx ? {...item, name: e.target.value} : item)}))} className="flex-1 border-2 border-black px-3 text-[10px] font-black uppercase" />
                                    </div>
                                        ...
                                        {(["shortImage", "longImage"] as const).map(imgKey => (
                                            (sleeves.enabled ? sleeves.types[imgKey === 'shortImage' ? 'short' : 'long'] : true) && (
                                                <div key={imgKey} className="aspect-square border-2 border-black bg-zinc-100 relative group overflow-hidden">
                                                    {(c as any)[imgKey] ? (
                                                        <>
                                                            <img src={typeof (c as any)[imgKey] === 'string' ? (c as any)[imgKey] : (c as any)[imgKey].preview} className="w-full h-full object-cover" />
        ...
        <div className={sectionClass}>
             ...
             <div className="flex items-center justify-between bg-zinc-100 border-2 border-black p-4 mb-6">
                <div>
                    ...
                    <input type="checkbox" {...register("isPreOrder")} className="ml-4 w-5 h-5 accent-black" />
                </div>
             </div>
        ...
        <div className="flex justify-end pt-8">
            <button 
                type="submit" 
                disabled={saving}
                className="bg-black text-white px-12 py-6 font-black uppercase tracking-[0.2em] text-xl border-4 border-black hover:bg-white hover:text-black transition-all shadow-[8px_8px_0px_0px_rgba(255,49,49,1)] hover:shadow-none translate-y-[-4px] active:translate-y-0"
            >
                {saving ? "DATA_STREAMING..." : "COMMIT_MODIFICATIONS"}
            </button>
        </div>
      </form>
    </div>
